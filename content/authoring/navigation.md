...
the "Getting Started" group in this sidebar shows both. An icon value is one of:

- **A Font Awesome class** — `fa-solid fa-rocket`, `fab fa-github`, or a bare name
  like `rocket` (which expands to `fa-solid fa-rocket`).
- **A path to an image** — any `.svg`/`.png`/`.jpg`/… under `static/`, e.g.
  `/icons/folder.svg`. Square images read best; the icon is capped to one line so
  it never makes the nav row taller.

```yaml
---
title: Quickstart
menu: docs
icon: fa-solid fa-bolt        # left of this page's label
heading: Getting Started
heading-icon: fa-solid fa-flag # left of the section label above it
---
```

Font Awesome glyphs need the FA stylesheet loaded. The quickest way is to let aardvark
inject it — set `theme.fontawesome: true` in `aardvark.config.yaml` (this site does) —
or pass a kit/CDN URL string to self-host or use Pro. Image-path icons need none
of that.
