...
spacing, `id`, and more. See its page, [Button](/components/button/), for the full
...
The other built-in is **`{% raw %}{% aside %}{% endraw %}`** — a static callout (or
admonition): set `title` for an optional bold heading and `color` to tint the left bar
(defaults to `primary`). See its page, [Aside](/components/aside/), for parameters and
examples.
...
- **Snippet** (`snippets/*.jsx`) — a genuinely new **React component** written in
  ...
  client-side. See [Components & snippets](/authoring/components-and-snippets/).
