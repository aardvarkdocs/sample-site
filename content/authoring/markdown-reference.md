...
aardvark renders every Markdown blockquote as a callout island rather than a plain
`<blockquote>`. A bare `>` becomes the same untitled notification that
[`{% raw %}{% aside %}{% endraw %}`](/components/aside/) emits, and the body is full Markdown:
...
`[!WARNING]`, or `[!CAUTION]` — and it renders as a [callout](/components/callout/)
...
Top-level numbered lists render as a [Steps](/components/steps/) timeline by default
...
[`{% raw %}{% divider %}{% endraw %}`](/components/divider/) tag.
...
{% aside title="Heads up" color="secondary" %}
