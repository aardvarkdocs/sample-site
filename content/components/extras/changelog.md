...
**On this page** list. This site's live [Changelog](/changelog/) tab is now
**taxonomy-driven** — the same timeline, fed from member articles by the
[`{% raw %}{% taxonomy %}{% endraw %}`](/components/extras/taxonomy/) tag — while
`{% raw %}{% changelog %}{% endraw %}` remains fully supported for YAML-file changelogs
like the live examples on this page.
...
Every changelog publishes an **RSS feed** at `feed.xml` beside the page, with one item per
change. (The [Changelog](/changelog/) tab's `/changelog/feed.xml` is published the same way
by its [taxonomy listing](/components/extras/taxonomy/).)
...
with its taxonomy-driven equivalent:
...
rendered changelog root. (Style it through the CSS parts above, and shape the feed with the
documented attributes: `tags`, `wide`, `combineByDay`, `limitDays`, `limitEntries`.)

{% changelog "data/changelog.yaml" limitEntries=3 toc=false attr={'data-analytics': 'release-feed', 'aria-label': 'Changelog'} %}
