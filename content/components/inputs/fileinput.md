...
Target a `{% raw %}{% fileinput %}{% endraw %}` from your own CSS with the island data attribute or the Mantine Styles API part classes:
...
{% accordionSection title="Source: Python" %}
```python
component('aardvark', 'fileinput', label='Resume', placeholder='Choose a file', attr={'onchange': '''
const value = Array.from(event.target.files || []).map((file) => file.name).join(', ');
console.log('attr demo value:', value);
alert(value || 'No file selected');
'''})
