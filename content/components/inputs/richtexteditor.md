...
handler — lands directly on the `.mantine-RichTextEditor-root` element:

{% RichTextEditor content='<p>x</p>' attr={'onclick': '''
...
{% accordionSection title="Source: Python" %}
```python
component('RichTextEditor', content='<p>x</p>', attr={'onclick': '''
const value = this.tagName;
console.log('attr demo value:', value);
alert(value);
'''})
