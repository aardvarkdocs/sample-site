...
Pass `attr={…}` to forward raw HTML attributes — including inline event handlers — straight onto the rendered element. Mantine forwards the ref to the component's wrapper here (not the inner control), so this demo reads `this.tagName` — `attr` still lands on the rendered node:

{% rangeslider min=0 max=100 defaultValue='20, 80' attr={'onclick': '''
...
{% accordionSection title="Source: Python" %}
```python
component('aardvark', 'rangeslider', min=0, max=100, defaultValue='20, 80', attr={'onclick': '''
const value = this.tagName;
console.log('attr demo value:', value);
alert(value);
'''})
