...
calls the gateway, which proxies the request to [OpenRouter](https://openrouter.ai/) behind a
single managed key, **meters** the spend (at OpenRouter cost x 1.5), and **cuts off** an account
...
Every account holds a **prepaid balance**. Each metered chat request **debits** the balance by the
model's OpenRouter cost times the gateway's markup. When the balance is exhausted, the gateway stops
serving that account's chat requests until it is funded again — so spend can never run away past
...
2. Create a Stripe **webhook** pointing at `…/v1/stripe/webhook`, subscribed to the
   `payment_intent.succeeded` and `payment_intent.payment_failed` events (the webhook is the durable
   credit backstop).
