---
description: Opt-in, cached OpenRouter features — generate frontmatter, example API
  responses, and Claude Code skills from your docs, using any model.
...
aardvark has optional, build-time features powered by [OpenRouter](https://openrouter.ai/),
so you can use **any model**. They are **off by default**, run only when explicitly
enabled, and their results are **cached** by content hash in `.aardvark-cache/ai/` — so
unchanged content never re-calls the API and builds stay fast and deterministic.

## Enable

1. Install the extra and set a key:
...
2. Turn on the features you want in `aardvark.config.yaml`, and pick a model:
   ...
   ai:
     ...
     model: "~anthropic/claude-sonnet-latest"   # any OpenRouter slug; "latest" alias needs the leading ~
   ```

   Any run can override the configured model with `--model <slug>` (e.g.
   `vark build --model openai/gpt-latest`). If the key or the SDK is missing, the
   features simply no-op.
...
- **`examples`** — generates a realistic example response for each OpenAPI
  operation and shows it on the [reference page](/openapi/).
