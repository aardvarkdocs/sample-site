...
(function () {
  ...
  function installApp(btn) {
    ...
    // The target is a build-time attribute (a root-relative page URL); fall back to the
    // standalone assistant page. Navigate only to a genuine same-origin path ("/…", but not
    // protocol-relative "//host"), so a non-path value can never redirect off-origin.
    var target = btn.getAttribute('data-aardvark-install-app') || '/_assistant/';
    if (target.charAt(0) !== '/' || target.charAt(1) === '/') target = '/_assistant/';
